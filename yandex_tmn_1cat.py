#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Yandex Maps – SCHOOLS ONLY (Tyumen)
# Same engine as yandex_tmn_8cat v8.8 (infinite scroll, /org/ parsing,
# dedup by org_id, coords from embedded JSON + per-org click fallback).
#
# IMPORTANT: uses the EXACT URL the user provided (with l=stv,sta, sctx=..., z=12)
# because only at that map position/context all schools are listed (not just 11).
# The URL is used verbatim and is NOT rebuilt from coordinates.
#
# Usage:
#   python yandex_schools.py --ru        # (default domain from the exact URL is .com)
#   python yandex_schools.py             # uses the exact URL as-is (yandex.com)
#   python yandex_schools.py --dry-run

import argparse, time, re, json, csv, sys
from pathlib import Path

# --- EXACT URL provided by the user (do not rebuild) ---
SCHOOLS_URL = (
...
META = {"key": "school", "name_ru": "Школы", "display_en": "Schools"}
...
def main():
    p = argparse.ArgumentParser(description="Yandex SCHOOLS (exact URL)")
    p.add_argument("--url", default=SCHOOLS_URL,
                   help="override the exact schools URL (default = the provided one)")
    p.add_argument("--headless", dest="headless", action="store_true")
    p.add_argument("--no-headless", dest="headless", action="store_false")
    p.set_defaults(headless=False)
    p.add_argument("--ru", action="store_true", help="rewrite domain to yandex.ru (keeps rest of URL)")
    ...
    url = args.url
    if args.ru:
        url = url.replace("yandex.com", "yandex.ru")
    domain = domain_of(url)

    print("=== YANDEX SCHOOLS (exact URL) ===")
    print(f"domain={domain}")
    print(f"URL: {url}")

    Path("links_schools_tyumen.txt").write_text(url + "\n", encoding="utf-8")
    ...
    with sync_playwright() as sp:
        ...
        if looks_like_captcha(page):
            ...
            if looks_like_captcha(page):
                print("  still captcha, aborting."); browser.close(); return
    ...
    out_csv = "results_schools_tyumen.csv"
    out_json = "results_schools_tyumen.json"
    ...
    if results:
        ...
        print(f"\n✓ {len(results)} schools  ({with_coords} with coordinates)")
        print(f"CSV : {out_csv}")
        print(f"JSON: {out_json}")
    else:
        print("\n0 rows – likely captcha / empty list. Keep headed, don't change the URL.")
